...
public class TimeUtil {
    ...
    public TimeUtil(String time, String timeSeparator) {
        ...
        String matcher = new StringBuffer().append("\\d+?").append(pattern).append("\\d+?").append(pattern).append("\\d+?").toString();
     ...
     * @return 处理后的字符
     */
    private String patternQuote(String param) {
        int slashEIndex = param.indexOf("\\E");
        if (slashEIndex == -1) {
            return "\\Q" + param + "\\E";
        }
        ...
        slashEIndex = 0;
        int current = 0;
        while ((slashEIndex = param.indexOf("\\E", current)) != -1) {
            sb.append(param.substring(current, slashEIndex));
            current = slashEIndex + 2;
            sb.append("\\E\\\\E\\Q");
        }
        sb.append(param.substring(current, param.length()));
        sb.append("\\E");
        return sb.toString();
    }
