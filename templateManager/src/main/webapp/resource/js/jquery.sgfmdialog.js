// JavaScript Document
(function ($) {
    var instances,max_zindex = 0,dialogs=[],isIE6=(/msie/.test(navigator.userAgent.toLowerCase()))&&/msie 6\.0/i.test(navigator.userAgent),//判断是否IE6浏览器
		jXHR={},//正在请求的XMLHttpRequest对象
		info={"rtCode":"返回失败返回码：","noMsg":"不存在对话框内容","hasDia":"对话框已经打开",
				"errorMsg":"错误信息","warnMsg":"警告信息","tipMsg":"提示信息","confirm":"确 定","cancel":"消 息"};//默认信息
     ...
     function init(settings,callback){
        ...
        if($this.length === 0){
            ...
            if($this.is(":visible")){
                /* todo 错误信息进行处理 */
                 if(settings.haserrmsg){
                 	if(typeof settings.controlMsg == 'string'){
                     	$.sgfmdialog(settings.controlMsg);
                 	}else{
